...
<html lang="en">
  ...
  <body>
			  ...
			  <div class="form-group" id="form-group-edit" style="text-align:center">
				<div id="form-group-salah">
					 <form method="POST" action="upload">
						 <input type="text" name="labelBenar" placeholder="label benar">
						 <input type="hidden" name="islink" value="nol">
						 <input type="hidden" name="update" value="satu">
						 <input type="text" name="konten" value="<% if(request.getAttribute("konten") != null){ out.println(request.getAttribute("konten"));} %>">
						 <input type="text" name="judul" value="<% if(request.getAttribute("judul") != null){ out.println(request.getAttribute("judul"));} %>">
	...
	function clickSingle(){
		document.getElementById("form-group-edit").innerHTML="<form id=\"form-input\" action=\"upload\" method=\"POST\" role=\"form\"><center><input type=\"text\" class=\"form-control\" id=\"textInput\" name=\"textInput\" placeholder=\"judul di sini\"><br/><textarea class=\"form-control\" id=\"textInput\" name=\"textInputKonten\"></textarea><br/><input type=\"hidden\" name=\"islink\" value=\"nol\"><button type=\"submit\" class=\"btn btn-default\" name=\"submit\" onclick=\"\">Submit</button></center></form>";
	}
	function clickMulti(){
		document.getElementById("form-group-edit").innerHTML="<form id=\"form-input\" action=\"upload\" method=\"POST\" role=\"form\" enctype=\"multipart/form-data\"><center><input type=\"file\" id=\"fileSelect\" name=\"file\"><br/><button type=\"submit\" class=\"btn btn-default\" name=\"submit\" onclick=\"\">Submit</button></center></form>";
	}
	function clickLink(){
